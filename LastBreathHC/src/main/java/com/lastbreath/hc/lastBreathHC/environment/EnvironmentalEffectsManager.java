...
public class EnvironmentalEffectsManager implements Listener {
    ...
    private final NamespacedKey deepOceanSwimSlowKey;
    ...
    public EnvironmentalEffectsManager(LastBreathHC plugin) {
        ...
        this.deepOceanSwimSlowKey = new NamespacedKey(plugin, "deep_ocean_swim_slow");

        ConfigurationSection config = plugin.getConfig().getConfigurationSection("environment");
        if (config == null) config = plugin.getConfig().createSection("environment");

        ConfigurationSection distanceSection = config.getConfigurationSection("distanceScaling");
        if (distanceSection == null) distanceSection = config.createSection("distanceScaling");
        ...
        if (biomeSection == null) biomeSection = config.createSection("biomes");

        ConfigurationSection swampSection = biomeSection.getConfigurationSection("swamp");
        if (swampSection == null) swampSection = biomeSection.createSection("swamp");
        ...
        if (desertSection == null) desertSection = biomeSection.createSection("desert");
        ...
        if (mountainSection == null) mountainSection = biomeSection.createSection("mountain");
        ...
        if (oceanSection == null) oceanSection = biomeSection.createSection("deepOcean");
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (player.hasMetadata("lb_last_move")) {
            ...
            if (now - last < moveCooldownTicks) return;
        }
        player.setMetadata("lb_last_move",
    ...
    public void onPotionSplash(PotionSplashEvent event) {
        ...
        for (LivingEntity entity : event.getAffectedEntities()) {
            ...
            for (PotionEffect effect : event.getPotion().getEffects()) {
                if (!DECAY_POTIONS.contains(effect.getType())) continue;
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        AttributeInstance attack = drowned.getAttribute(Attribute.ATTACK_DAMAGE);
        if (attack != null) attack.setBaseValue(attack.getBaseValue() * scale);

        AttributeInstance health = drowned.getAttribute(Attribute.MAX_HEALTH);
        if (health != null) {
    ...
    private void applyFireScaling(Player player, double scale) {
        ...
        if (adjusted != fireTicks) player.setFireTicks(adjusted);
    }
    ...
    private void applyPotionDecay(Player player, double scale) {
        for (PotionEffect effect : player.getActivePotionEffects()) {
            ...
            int adjusted = adjustDurationForInterval(effect.getDuration(), scale);
            if (adjusted != effect.getDuration()) {
    ...
    private void applyBiomeEffects(Player player, Location location) {
        String biomeKey = location.getBlock().getBiome()
                .getKey().getKey().toUpperCase(Locale.ROOT);

        if (swampBiomes.contains(biomeKey)) {
            if (ThreadLocalRandom.current().nextDouble() <= swampPoisonChance) {
                player.addPotionEffect(new PotionEffect(
                        PotionEffectType.POISON,
        ...
        if (desertBiomes.contains(biomeKey)) {
            if (isIdle(player)
                    && ThreadLocalRandom.current().nextDouble() <= desertHeatChance) {
    ...
    private void updateSwimSlow(Player player, boolean inDeepOcean) {
        ...
        AttributeModifier existing = movement.getModifier(deepOceanSwimSlowKey);
    ...
    private boolean isIdle(Player player) {
        if (!player.hasMetadata("lb_last_move")) return true;
        int last = player.getMetadata("lb_last_move").getFirst().asInt();
        return (player.getTicksLived() - last) >= desertIdleThresholdTicks;
    }
    ...
    private double distanceScale(Location location) {
        ...
        double t = Math.min(distance, spawnRadiusBlocks) / spawnRadiusBlocks;
    ...
    private Set<String> parseBiomeList(java.util.List<String> values, Set<String> defaults) {
        ...
        for (String value : values) {
            if (value != null) result.add(value.trim().toUpperCase(Locale.ROOT));
        }
